    ...
    class Program
    {
        ...
        static async Task SimulateTransactionsAsync(List<BankAccount> accounts, int numberOfTransactions, double minTransactionAmount, double maxTransactionAmount)
        {
            var tasks = accounts.Select(async account =>
            {
                for (int i = 0; i < numberOfTransactions; i++)
                {
                    ...
                    try
                    {
                        if (transactionAmount >= 0)
                        {
                            ...
                            Console.WriteLine($"Credit recorded. Account: {MaskAccountNumber(account.AccountNumber)}, Holder: {MaskName(account.AccountHolderName)}, Type: {account.AccountType}");
                        }
                        else
                        {
                            account.Debit(-transactionAmount);
                            Console.WriteLine($"Debit recorded. Account: {MaskAccountNumber(account.AccountNumber)}, Holder: {MaskName(account.AccountHolderName)}, Type: {account.AccountType}");
                        }
                    }
                    catch (Exception ex)
                    {
                ...
                Console.WriteLine($"Summary. Account: {MaskAccountNumber(account.AccountNumber)}, Holder: {MaskName(account.AccountHolderName)}, Type: {account.AccountType}");
            });
        ...
        {
            return string.Join(".", name.Split(' ').Select(p => p.Length > 0 ? p[0].ToString() : "")) + ".";
        }
